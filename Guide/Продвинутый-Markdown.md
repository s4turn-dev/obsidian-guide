...
Ссылки - это фундамент Obsidian, именно они превращают набор заметок в связанную базу знаний.

**Ссылки на другие заметки:**

- **Wiki-стиль:** `[[Название заметки]]`. Это самый простой способ. Просто пишешь название заметки в двойных квадратных скобках, и Obsidian сам найдет ее. Если заметки нет, он предложит ее создать.

- **Стандартный Markdown-стиль:** `[текст ссылки](путь/к/заметке.md)`. Этот стиль более универсален и работает не только в Obsidian, но и в других Markdown-редакторах. `путь/к/заметке.md` — это полный путь к файлу относительно корня твоего хранилища.

Совет: в настройках Obsidian в разделе "Файлы и ссылки" есть опция "Использовать вики-ссылки". Рекомендуем включить данные ссылки, их стиль проще и понятнее.
...
Бывает, что содержимое ссылки нужно видеть "здесь и сейчас", тогда на помощь приходит символ `!`. Он позволяет отобразить содержимое ссылки прямо в текущей заметке.
 ...
 - **Вставить отдельный параграф (блок заметки):** ...

Отличная штука для создания сводок или для сборка информации из разных источников в одном месте без копипаста 😉. Если изменится оригинал - изменится и вставленное содержимое.
