...
- **Перемещение по таблице:** На `<Tab>` и `<Shift>+<Tab>` ты можешь перемещать курсор вперед/назад соответственно между ячейками таблицы. На `<Enter>` ты перенесешь курсор в ячейку под текущей.
...
- Если заголовок находится в той же заметке: `[[#Заголовок_раздела]]`. Также через знак `|` можешь отображаемый текст ссылки: `[[Лекция_5#Введение|К введению Лекции 5]]`.
...
Для того. чтобы ссылка на отдельный параграф появилась, после него нужно оставить особый "якорь", тег, который может выглядеть вот так: `^151gho`
...
Этот тег -- самый обыкновенный текст: `...цифровые конспекты. ^ce09f0`. Не затри его случайно, иначе все ссылки, которые используют этот тег сломаются.
...
Бывает, что содержимое ссылки нужно видеть "здесь и сейчас", тогда на помощь приходит символ `!`. Он позволяет отобразить содержимое ссылки прямо в текущей заметке.
 ...
 - **Вставить отдельный параграф:** `![[Название-заметки#^6230cd]]` -- вставит только один абзац, на который ссылается тег.

Пример: ![[#^3d1f9e]]


### Бонус: ссылки в текущей заметке

Если ты хочешь вставить ссылку на заголовок или абзац в *текущей* заметке, то название заметки можно не вводить: `[[#Больше выделения, или HTML]]`, `[[#^3d1f9e]]`.
